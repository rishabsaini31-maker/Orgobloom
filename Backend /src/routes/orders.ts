...
// Track order (public endpoint - by order number)
router.get(
  "/track/:orderNumber",
  async (req: any, res: Response, next: NextFunction) => {
    try {
      ...
      const trackingNumber = `ORG${order.id.slice(0, 10).toUpperCase()}${Math.random()
        .toString(36)
        .substring(7)
        .toUpperCase()}`;

      // Generate estimated delivery (mock - 5-7 days from now)
