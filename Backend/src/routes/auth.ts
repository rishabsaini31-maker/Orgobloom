...
const loginLimiter = rateLimit({
  ...
  keyGenerator: (req, res) => {
    ...
    console.log(`[RATE LIMIT DEBUG] Login attempt from IP: ${ip}`);
    return ip;
  },
  handler: (req, res) => {
    const ip = getClientIP(req);
    console.log(
      `[RATE LIMIT] ❌ Login blocked for IP: ${ip} - Too many attempts`,
    );
    res.status(429).json({
      error: "Too many login attempts",
      message: "Please try again after 15 minutes",
...
const registerLimiter = rateLimit({
  ...
  keyGenerator: (req, res) => {
    const ip = getClientIP(req);
    console.log(`[RATE LIMIT DEBUG] Register attempt from IP: ${ip}`);
    return ip;
  },
  handler: (req, res) => {
    const ip = getClientIP(req);
    console.log(
      `[RATE LIMIT] ❌ Register blocked for IP: ${ip} - Too many attempts`,
    );
    res.status(429).json({
      error: "Too many registration attempts",
      message: "Please try again after 1 hour",
